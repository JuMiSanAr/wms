...
class ShopfloorLog(models.Model):
    ...
    def _get_severity(self):
        if not self.exception_name:
            return False
        mapping = self._get_exception_mapping()
        return mapping.get(self.exception_name, "warning")

    def _get_severity_exception_mapping_param(self):
        param = (
    ...
    @tools.ormcache("self._get_severity_exception_mapping_param()")
    def _get_exception_mapping(self):
        mapping = self.EXCEPTION_SEVERITY_MAPPING.copy()
        param = self._get_severity_exception_mapping_param()
        if not param:
            return mapping
