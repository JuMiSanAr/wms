...
class ShopfloorApp(models.Model):
    ...
    shop_api_route_public = fields.Char(compute="_compute_shop_api_route")
    ...
    @api.depends("api_route", "api_route_public")
    def _compute_shop_api_route(self):
        for rec in self:
            rec.shop_api_route = rec.api_route.rstrip("/") + "/invader/"
            rec.shop_api_route_public = rec.api_route_public.rstrip("/") + "/invader/"
